...
class Kinematics:
    ...
    def heading_callback(self, heading: Float32):
        if self.angleOffset == 0:
            self.angleOffset=np.deg2rad(heading.data)
        self.angle=np.deg2rad(heading.data)-self.angleOffset
...
if __name__ == '__main__':
    ...
    sub4 = rospy.Subscriber('heading', Float32,
                            callback=kinematics.heading_callback)
